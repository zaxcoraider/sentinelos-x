...
const AGENT_META: Record<string, { label: string; icon: LucideIcon; tone: string; color: string }> = {
  treasury: { label: 'Treasury Agent', icon: Coins, tone: '52, 211, 153', color: 'text-emerald-400' },
  governance: { label: 'Governance Agent', icon: Landmark, tone: '167, 139, 250', color: 'text-violet-400' },
};

function threatLevel(severity: number | null): { label: string; tone: string; rgb: string; dot: string } {
  if (severity === null) return { label: 'No signal', tone: 'text-muted-foreground', rgb: '148, 163, 184', dot: 'bg-muted-foreground' };
  if (severity >= 60) return { label: 'Elevated', tone: 'text-red-400', rgb: '239, 68, 68', dot: 'bg-red-500' };
  if (severity >= 30) return { label: 'Guarded', tone: 'text-amber-400', rgb: '251, 191, 36', dot: 'bg-amber-400' };
  return { label: 'Nominal', tone: 'text-emerald-400', rgb: '52, 211, 153', dot: 'bg-emerald-400' };
}
...
export function SecurityCenter({
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-6 p-6">
          <div className="flex items-center gap-4">
            <div
              ...
              style={{
                borderColor: `rgb(${threat.rgb} / 0.4)`,
                background: `rgb(${threat.rgb} / 0.08)`,
                boxShadow: `0 0 26px -8px rgb(${threat.rgb} / 0.7)`,
              }}
            >
              {peak !== null && peak >= 60 && (
                <motion.span
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-2 text-sm text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground"
          >
      ...
      <Panel
        title="On-chain activity · live from CSPR.cloud"
        icon={Activity}
        tone="56, 189, 248"
        right={
